<?php

	$plugin_cf['register']['allowed_register']="true";
	$plugin_cf['register']['captcha_mode']="formula";
	$plugin_cf['register']['captcha_font']="Arcade Classic.ttf";
	$plugin_cf['register']['captcha_crypt']="registeruser";
	$plugin_cf['register']['captcha_image_width']="120";
	$plugin_cf['register']['captcha_image_height']="20";
	$plugin_cf['register']['captcha_chars']="2";
	$plugin_cf['register']['encrypt_password']="";
	$plugin_cf['register']['group_activated']="guest";
	$plugin_cf['register']['group_administrator']="admin";
	$plugin_cf['register']['group_default']="guest";
	$plugin_cf['register']['hide_pages']="true";
	$plugin_cf['register']['image_login']="submit_blue.gif";
	$plugin_cf['register']['image_logout']="logout_blue.png";
	$plugin_cf['register']['image_forgot_password']="forgot_blue.png";
	$plugin_cf['register']['image_user_preferences']="preferences_blue.png";
	$plugin_cf['register']['logfile']="true";
	$plugin_cf['register']['login_layout']="";
	$plugin_cf['register']['login_all_subsites']="";
	$plugin_cf['register']['remember_user']="true";
	$plugin_cf['register']['senderemail']="e-mail adresse";
	$plugin_cf['register']['utf8_check']="äöü";

?>