...
class UserAdmin
{
    ...
    private function overview(Request $request, array $errors = []): Response
    {
        ...
        $users = Users::retrieve($this->store)->users($filters);
    ...
    private function doCreate(Request $request): Response
    {
        ...
        $user->setSecret(base64_encode($this->random->bytes(15)));
    ...
    private function doUpdate(Request $request): Response
    {
        ...
        $post = [
            ...
            "groups" => array_values($request->postArray("groups") ?? []),
            "status" => $request->post("status") ?? "",
        ];
        $duplicateEmail = $users->userByEmail($post["email"]) !== $user;
    ...
    private function doMail(Request $request): Response
    {
        ...
        $mail = new Mail($request->post("subject") ?? "", $request->post("message") ?? "");
    ...
    private function sendMail(User $user, Mail $mail, string $from): bool
    {
